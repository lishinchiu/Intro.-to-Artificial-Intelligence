...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax(self, gameState, depth, agentIdx, maximize):
        ...
        if maximize: 
            for move in moves:
                temp.append(self.minimax(gameState.getNextState(agentIdx, move), depth-1, 1, False))
            stateScore = max(temp, key=lambda item: item[1]) 
        elif agentIdx < gameState.getNumAgents()-1: 
            for move in moves:
                temp.append(self.minimax(gameState.getNextState(agentIdx, move), depth, agentIdx+1, False))
            stateScore = min(temp, key=lambda item: item[1]) 
        else: 
            for move in moves:
                temp.append(self.minimax(gameState.getNextState(agentIdx, move), depth, 0, True))
            stateScore = min(temp, key=lambda item: item[1]) 
        return stateScore 
        # End your code
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        temp = []
        for move in moves:
            temp.append((move, self.expectimax(gameState.getNextState(0, move), self.depth-1, 1, False)))
        move,  _= max(temp, key=lambda item: item[1])
        return move  
    ...
    def expectimax(self, gameState, depth, agentIdx, maximize):
        ...
        if maximize:
            for action in actions:
                temp.append(self.expectimax(gameState.getNextState(agentIdx, action), depth-1, 1, False))
            score = max(temp)
        elif agentIdx < gameState.getNumAgents()-1:
            tmp = 0 
            for action in actions:
