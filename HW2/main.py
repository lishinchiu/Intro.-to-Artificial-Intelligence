...
def second_part(model_type, df_train, df_test, N):
    ...
    if model_type == 'ngram':
        ...
        test_dataloader = DataLoader(test_data, collate_fn=collate_fn, batch_size=rnn_config['batch_size'], shuffle=False)
...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in range(epochs):
        ...
        for batch_inputs, batch_labels in tqdm(train_dataloader, desc=f"Epoch {epoch + 1} Training"):
        ...
        with torch.no_grad():
            for batch_inputs, batch_labels in tqdm(test_dataloader, desc=f"Epoch {epoch + 1} Testing"):
        ...
        print(f"Epoch: {epoch + 1}, F1 score: {f1}, Precision: {precision}, Recall: {recall}, Loss: {avg_loss}")
